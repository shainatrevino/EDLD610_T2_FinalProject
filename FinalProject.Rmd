...
plot2 <- ggplot(plot2data, aes(x = word, y = mean_TEPR, color = span_group)) +
...
Here I re-create the plot above but with geom_ribbon instead of geom_smooth.. I can't decide which approach is best.

```{r plot2_geom_ribbon, fig.height=5, fig.width=6.5, fig.align='center'}

#my_colors = carto_pal(3, "Burg")
#my_colors

plot2.2 <- ggplot(plot2data, aes(x = word, y = mean_TEPR, color = span_group)) +
...
plot3 <- plot3_data %>%
  ...
  summarise(mean_TEPR = mean(TEPR, na.rm = TRUE),
            sd_TEPR = sd(TEPR, na.rm = TRUE)) %>%
  ggplot(aes(x = bin, y = mean_TEPR, color = span_group)) +
      geom_smooth() +
      scale_color_carto_d(palette = "Burg") +
      facet_wrap(~word) +
      theme_minimal() +
...
For the original figure in Miller, Gross, and Unsworth, I faceted by WMC rather than serial position. Let's try to re-create that plot and compare (note, I think I prefer the above where I facet by serial position):

```{r plot3_version2, fig.align='center'}

plot3.2 <- plot3_data %>%
  group_by(word, bin, span_group) %>%
  summarise(mean_TEPR = mean(TEPR, na.rm = TRUE),
            sd_TEPR = sd(TEPR, na.rm = TRUE)) %>%
  ggplot(aes(x = bin, y = mean_TEPR, color = word)) +
      geom_smooth() +
      scale_color_carto_d(palette = "Burg") +
...
plot4 <- ggplot(plot4, aes(score, mean_acc)) +
    ...
    theme(axis.title = element_text(face = "bold"),
      ...
      labs(title = "Mean recall accuracy as a function of effective strategy use",
           y = "Recall Accuracy",
